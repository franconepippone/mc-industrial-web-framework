...
When a non‑RDS‑compliant network must interoperate with an RDS‑compliant one, dedicated **Protocol Adapters** are required. These components, typically placed at the link level, translate Address Stamps, routing rules, potentially package tecnhology between the two protocols. This makes mixed‑protocol environments technically possible, but significantly increases the complexity and (possibly) latency of the overall system. Unless external constraints mandate the use of a custom protocol, relying on the Standard RDS Protocol for maximum compliancy is recommended.
